...
export class CompareFileWithServerCommand implements ICommand {
    ...
    public execute(fileUri: Uri, _props?: any): Thenable<any> {
        try {
            ...
            return UiHelper.showStatusBarProgress(
                `Downloading Server version of: ${fileName}`,
                vscode.window.spgo
                    .initialize(fileUri)
                    .then((config: IConfig) => {
                        // is this a directory?
                        if (fs.lstatSync(fileUri.fsPath).isDirectory()) {
                            Logger.showWarning(
                                'The compare file with server command only works for single files at this time.'
                            );
                        } else {
                            if (fileUri.fsPath.includes(config.sourceRoot)) {
                                ...
                                Logger.outputMessage(
                                    `Starting file compare for: ${fileName}`,
                                ...
                                return AuthenticationService.verifyCredentials(vscode.window.spgo, config, fileUri)
                                    .then(
                                        async (fileUri) =>
                                            await fileService.downloadFileMajorVersion(fileUri, downloadPath)
                                    )
                                    .then((dlFileUrl) => {
                                        this.openFileCompare(fileUri, dlFileUrl);
                                    })
                                    .then(() => {
    ...
    public openFileCompare(fileUri: vscode.Uri, dlFileUrl: any) {
        if (dlFileUrl && dlFileUrl.length > 0) {
            const remotePath: vscode.Uri = vscode.Uri.parse(
                'file:///' + UrlHelper.removeLeadingSlash(dlFileUrl[0].SavedToLocalPath)
            );
