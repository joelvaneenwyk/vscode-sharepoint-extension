...
export class ConfigureWorkspaceCommand implements ICommand {
    ...
    public async getSharePointSiteUrl(fileUri: Uri): Promise<any> {
        return await new Promise((resolve, reject) => {
            const configFilePath: Uri = Uri.parse(
                vscode.workspace.getWorkspaceFolder(fileUri).uri + '/' + Constants.CONFIG_FILE_NAME
            );
    ...
    public getPublishingScope(config: IConfig): Thenable<any> {
        ...
        const options: vscode.QuickPickItem[] = [
            {
                description:
                    'Save the file to the server, but do not publish. You must manually publish via the VSCode menu or hotkey (alt+shift+p)',
            ...
            {
                description:
                    'Do not save the file to the server. You must manually publish via the VSCode menu or hotkey (alt+shift+p)',
                label: 'Do nothing',
    ...
    public finished(config: IConfig, rootPath: Uri): IConfig {
        ...
        fs.outputFile(
            rootPath.fsPath + path.sep + Constants.CONFIG_FILE_NAME,
