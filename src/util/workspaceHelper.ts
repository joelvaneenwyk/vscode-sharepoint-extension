...
export class WorkspaceHelper {
    static getActiveFile(): Uri {
        ...
        if (
            ...
            selectedResource = vscode.window.activeTextEditor.document
    ...
    static async getActiveWorkspaceUri(): Promise<Uri> {
        return await new Promise((resolve, reject) => {
            ...
            if (vscode.workspace.workspaceFolders.length > 1) {
                if (
                    ...
                    const selectedResource: Uri = vscode.window.activeTextEditor.document
                        ? vscode.window.activeTextEditor.document.uri
                        : Uri.parse('');
                    ...
                    return vscode.window
                        .showQuickPick(
                            vscode.workspace.workspaceFolders.map((folder: WorkspaceFolder) => {
    ...
    static getSiteUriForActiveWorkspace(fileUri: string, config: IConfig): Uri {
        // Promise<Uri> {

        const matchCount: number = fileUri.replace(config.sharePointSiteUrl, '').length;
