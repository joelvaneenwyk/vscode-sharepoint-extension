...
export class UrlHelper {
    ...
    public static getServerRelativeFilePath(fileName: string, config: IConfig): string {
        const relativeFilePath: string = this.getSiteRelativeFilePath(fileName, config);
        const remoteFolder = relativeFilePath.substring(0, relativeFilePath.lastIndexOf(path.sep));
        const remoteFileName = this.getFileName(relativeFilePath);
        const remoteFileUrl = this.normalizeSlashes(UrlHelper.formatWebFolder(remoteFolder) + remoteFileName);
    ...
    public static isFile(uri: string): boolean {
        ...
        return fileName.includes('.') && fileName != '.' && !fileName.includes('*');
    }
