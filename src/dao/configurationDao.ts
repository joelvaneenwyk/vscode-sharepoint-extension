...
export class ConfigurationDao {
    static async initializeConfiguration(contextPath: Uri): Promise<IConfig> {
        return await new Promise((resolve, reject) => {
            ...
            try {
                if (!vscode.workspace.workspaceFolders) {
                    throw {
                        message:
                            'In order to use SPGo, please create a local folder to use as the solution workspace. See https://www.chrishasz.com/spgo/general/getting-started-with-spgo for more info.'
                    };
                }

                const configFilePath: string = contextPath.fsPath;
                const rootPath: Uri = vscode.workspace.getWorkspaceFolder(contextPath).uri;
                    ...
                    try {
                        ...
                        if (config.publishWorkspaceGlobPattern) {
                            ...
                            Logger.outputMessage(
                                'The publishWorkspaceGlobPattern property has been deprecated. Please use publishWorkspaceOptions instead: https://www.chrishasz.com/spgo/general/config-options#publishWorkspaceOptions'
                            );
                        }
                        ...
                        config.publishWorkspaceOptions.destinationFolder =
                            config.publishWorkspaceOptions.destinationFolder || '/';
                        config.publishWorkspaceOptions.localRoot = this.calculateLocalRoot(config);
                        config.publishWorkspaceOptions.globPattern = this.calculateGlobPattern(config);
    ...
    static calculateGlobPattern(config: IConfig): string[] {
        const globPattern: string[] = []; // = [config.publishWorkspaceOptions.globPattern];
        ...
        if (config.publishWorkspaceOptions.globPattern) {
            if (Array.isArray(config.publishWorkspaceOptions.globPattern)) {
                ...
                if (!path.isAbsolute(config.publishWorkspaceOptions.globPattern)) {
                    globPattern.push(
                        config.publishWorkspaceOptions.localRoot +
                            UrlHelper.ensureLeadingWebSlash(config.publishWorkspaceOptions.globPattern)
                    );
                }
            }
        } else {
