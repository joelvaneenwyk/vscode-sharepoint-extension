...
export class CredentialDao {
    ...
    static getCredentials(name: string): ICredential {
        ...
        try {
            ...
            if (retrievedCredentials) {
                retrievedCredentials.username = retrievedCredentials.username
                    ? cpass.decode(retrievedCredentials.username)
                    : null;
                retrievedCredentials.password = retrievedCredentials.password
                    ? cpass.decode(retrievedCredentials.password)
                    : null;
                retrievedCredentials.clientId = retrievedCredentials.clientId
                    ? cpass.decode(retrievedCredentials.clientId)
                    : null;
                retrievedCredentials.clientSecret = retrievedCredentials.clientSecret
                    ? cpass.decode(retrievedCredentials.clientSecret)
                    : null;
                retrievedCredentials.realm = retrievedCredentials.realm
                    ? cpass.decode(retrievedCredentials.realm)
                    : null;
                retrievedCredentials.domain = retrievedCredentials.domain
                    ? cpass.decode(retrievedCredentials.domain)
                    : null;
            }
        } catch (err) {
    ...
    static setCredentials(name: string, credentials: ICredential) {
        ...
        const storedCredentials: ICredential = {
